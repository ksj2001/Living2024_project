...
public class LivingDAO {
    ...
    public int getCodeCount(int code) {
    	...
    	int count = 0;
    	try {
    		String sql = "select count(*) from cart C inner join product P on P.p_code = C.p_code where P.p_code=?";
    		pstmt = con.prepareStatement(sql);
    		pstmt.setInt(1, code);
    		rs = pstmt.executeQuery();
    		if(rs.next()) {
    ...
    public void updateCart1(int cnt, int code) {
    	getConnect();
    	try {
    		String sql = "update cart C inner join product P on P.p_code = C.p_code \r\n"
    				+ "set C.c_qty=C.c_qty+?, C.c_total=C.c_qty*P.p_price where C.c_code=?";
    		pstmt = con.prepareStatement(sql);
    		pstmt.setInt(1, cnt);
    		pstmt.setInt(2, code);
    		pstmt.executeUpdate();
